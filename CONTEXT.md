...
**identity の値域**: identity を構成する JSON 値の許容域（spec §5・→ ADR-0024）。文字列（全 Unicode）/ 整数（±(2^53−1)、整数表記のみ）/ bool / null / 配列 / オブジェクト（メンバー名は ASCII）。数値は表記で判定し、小数点・指数表記（`1.0`・`1e3` 等）・NaN・Infinity は域外。域外は実装が id 導出時に拒否する。制約された値域の上で各実装はフル JCS と一致する。
