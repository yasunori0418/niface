...
**適合 (conformance)**: `schema/v1/` の JSON Schema 検証・schema で表現しきれない MUST のリント検査（status↔errors/item 整合・`changes[].itemId` の同一 result 内参照整合・`subject.name` / `item.id` の一意性）・`testdata/v1/id-vectors.json` の全ベクタ通過をもって判定する。schema 検証とリント検査の参照実装は `scripts/validate.py`（→ `spec/v1/spec.md` §8, ADR-0021）。
