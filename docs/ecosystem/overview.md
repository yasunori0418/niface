...
niface の **draft → stable** は **2 ツール適合**（nput + 次ツール、M2 の nboot 想定）で宣言する。宣言時に draft 表記を落とす。リリースは同一コミットに `v1.N.P`（規格スナップショット）と `go/v1.N.P`（Go module 用ミラー）を対で打ち、互換追加ごとに `N`、規格に影響しない修正で `P` を増やす（→ niface ADR-0026）。
...
3. **規格が契約**: ツールの実行結果は niface 規格（結果エンベロープ）のみに依存して消費できる。入力 JSON の形は各ツールが公開・管理し、ツール固有の知識は `info` に隔離される（→ niface ADR-0014）
