...
- go/conformance の `NewChecker` で `santhosh-tekuri/jsonschema` の `AssertFormat()` を呼び、format assertion を有効化する。santhosh-tekuri は `date-time` を RFC 3339 で検証する。既存 API の呼び出しのみで**依存追加なし → vendorHash 不変 → `flake.nix` 不変**。
...
- `date-time` の検証は santhosh-tekuri が RFC 3339 で行う。日時検証は適合検証器(Go)の領分で、Nix は id 導出のみを担い日時に関与しないため、値域変更のような Go/Nix/ベクタ 3 点同期の対象外。
...
- **spec §2 の例コメントのみで RFC 3339 を示し prose MUST を書かない**: 現状維持。第三者検証器の format 設定次第で適合判定が割れ、§8 の「schema 検証を通ること」が format を含むか曖昧なまま残る。prose MUST と §8 の assertion 明記で判定を一意にする。
