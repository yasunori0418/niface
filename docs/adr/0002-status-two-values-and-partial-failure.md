...
> **2026-07-07 改訂注記（ADR-0011）**: 本 ADR の status 2 値・「1 件でも item が failed なら error」・item status は不変。集約の範囲が `results[]` 全体に拡張され、いずれかの主体が `error` か主体列挙前の全体エラーがあれば最上位 `status` は `error`、対象 0 件の batch は `success` となった（→ ADR-0011）。
