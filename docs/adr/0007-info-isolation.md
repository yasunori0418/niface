...
> **2026-07-07 改訂注記（ADR-0018）**: 本 ADR の「item / change / result の 3 箇所」の列挙が対象。
> ツール固有情報を `info` 配下に隔離する原則と `additionalProperties: false` は不変。ADR-0018 でエンベロープ直下にも optional な `info` が追加され、置き場はエンベロープ最上位 / result / item / change の 4 箇所になった（→ ADR-0018）。
