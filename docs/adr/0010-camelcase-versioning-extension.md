# ADR-0010: 命名・バージョニング・拡張ポリシーを定める

- ステータス: 採用
- 日付: 2026-07-05
- 関連: `spec/v1/spec.md`（§2）, `schema/v1/envelope.schema.json`

> **2026-07-11 改訂注記（ADR-0024）**: 本 ADR の specVersion を上げる条件（削除・意味変更・必須化で increment、互換変更では増やさない）は不変。ただし、参照実装間で相互運用が成立していなかった領域（実装依存で id 等が割れていた入力域）を規範から除外・厳格化する変更は、それに安全に依存できた消費側が存在しないため互換扱いとし specVersion を上げない、という例外を追加した（→ ADR-0024・spec §2）。

> **2026-07-11 改訂注記（ADR-0026）**: 本 ADR の棄却案「semver 文字列」は `specVersion` フィールドの粒度に関する判断であり不変。リリースの識別子には別レイヤとして semver 形式のタグ `v1.N.P` / `go/v1.N.P` を採用する（→ ADR-0026）。`specVersion`（非互換世代を表す整数）とタグ（リリーススナップショット識別子）はレイヤが異なり、両者は矛盾しない。

> **2026-07-18 改訂注記（ADR-0027）**: specVersion 据え置きの互換扱いに第 2 の類型を追加した。全参照実装が既に一致した挙動を示している領域を、その挙動どおりに規範化（明文化）する変更は、実装挙動と id 導出結果が変わらず消費側の観測値も変わらないため、必須化（MUST の追加）を伴っても互換扱いとし specVersion を上げない（→ ADR-0027・spec §2）。ADR-0024 の追加した第 1 類型（相互運用が成立していなかった域の除外・厳格化）とは別類型で、両者は「安全に依存できた消費側が壊れない」点で共通する。

## 背景

フィールド命名規約、`specVersion` の運用、時刻の表現、拡張時の互換性という規約系の論点をまとめて確定する。これらは個別には小さいが、決めておかないと実装ごとにぶれる。

## 決定

- フィールド命名は camelCase とする（nput の `manifest.json` と整合）。
- `specVersion` は整数とする。
- 時刻は `startedAt` + `finishedAt`（RFC 3339）で表す。
- 消費側は未知フィールドを無視する（must-ignore）。
- 互換変更（フィールド追加）では `specVersion` を上げない。フィールドの削除・意味変更・必須化のときのみ上げ、その場合は `spec/v2/` を新設する。

## 根拠

- camelCase は JSON の一般的慣行であり、既存の nput `manifest.json` とも揃う。
- must-ignore により前方互換なフィールド追加が可能になり、`specVersion` のインフレを防げる。追加のたびに版を上げると消費側の追随を強制しエコシステムが分断する。
- 整数バージョンとディレクトリ（`v1/`）を一致させることで、参照とレイアウトが単純になる。

## 影響

- `spec/v1/spec.md` §2。
- `schema/v1/envelope.schema.json` 全体（camelCase / `specVersion` 整数 / 時刻）。
- ディレクトリ構成（`spec/v1/` / `schema/v1/` / `testdata/v1/`）。

## 棄却した代替案

- **snake_case**: nput `manifest.json` と不整合になる。
- **semver 文字列**: 規格のバージョン粒度に対して過剰。整数で足りる。
- **フィールド追加でも `specVersion` を上げる**: 消費側に追随を強制し、前方互換の利点を失う。
