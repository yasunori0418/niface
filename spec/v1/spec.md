...
- 文字列: 値は全 Unicode を許す
- 整数: −(2^53−1) 以上 +(2^53−1) 以下。範囲外の整数は域外とする
- 真偽値（bool）／ null
- 配列: 要素は本値域の値
- オブジェクト: 値は本値域の値。**メンバー名は ASCII（U+0000–U+007F）に限る**
- 非整数の数値（小数・NaN・Infinity 等）を identity に含めてはならない（MUST NOT）

整数の上限を安全整数（2^53−1、IEEE 754 倍精度が正確に表せる整数の上限）に揃えるのは、JSON の整数を double で読む実装との間で id が割れるのを防ぐためである。メンバー名を ASCII に限るのは、JCS のキー順序（UTF-16 code unit 順）と実装のキー順序（例: UTF-8 バイト順）が BMP 外の文字で食い違い、id が割れるのを避けるためである。文字列の値と配列・オブジェクトの値には非 ASCII を許す。key は宣言上の同一性を表す最小の値集合とする原則（本節）のもとで、この値域が実用上の制約になることはない。
