...
- `specVersion` は整数。互換変更（フィールド追加）では増やさない。フィールドの削除・意味変更・必須化で増やす。ただし、参照実装間で相互運用が成立していなかった領域（実装依存で id 等が割れていた入力域）を規範から除外・厳格化する変更は、それに安全に依存できた消費側が存在しないため互換扱いとし、`specVersion` を上げない（→ §5 identity の値域）
...
- `startedAt` / `finishedAt` は実行の開始・終了時刻を表す RFC 3339 の date-time 文字列でなければならない（MUST）。タイムゾーンオフセット（`Z` または `±hh:mm`）を含める。`subjectResult` の `startedAt` / `finishedAt`（各主体の実行時刻）も同じ制約に従う。schema は `format: date-time` でこれを表現し、適合検証は format を assertion として評価する（§8）
...
- 文字列: 値は全 Unicode を許す
- 数値: **整数表記**の整数のみを許し、値域は −(2^53−1) 以上 +(2^53−1) 以下とする。範囲外の整数、および小数点・指数を持つ表記（`1.0`・`1e3` 等、値が整数でも）・NaN・Infinity は域外とする（MUST NOT）
- 真偽値（bool）／ null
- 配列: 要素は本値域の値
- オブジェクト: 値は本値域の値。**メンバー名は ASCII（U+0000–U+007F）に限る**

数値は**値ではなく表記**で判定する。`1` と `1.0` は別の入力として扱い、値が整数でも小数点・指数表記（`1.0`・`1e3`）は域外とする。参照実装はこれを機械的に一致させる（Nix は `builtins.fromJSON` の int / float 型、Go は `encoding/json` の `UseNumber`（`json.Number` の字句）で表記を保持する）。整数の上限を安全整数（2^53−1、IEEE 754 倍精度が正確に表せる整数の上限）に揃えるのは、JSON の整数を double で読む実装との間で id が割れるのを防ぐためである。メンバー名を ASCII に限るのは、JCS のキー順序（UTF-16 code unit 順）と実装のキー順序（例: UTF-8 バイト順）が BMP 外の文字で食い違い、id が割れるのを避けるためである。文字列の値と配列・オブジェクトの値には非 ASCII を許す。key は宣言上の同一性を表す最小の値集合とする原則（本節）のもとで、この値域が実用上の制約になることはない。
...
- (1) の schema 検証は `format` を**単なる注釈ではなく assertion として評価する**。`format: date-time` を宣言した `startedAt` / `finishedAt` は RFC 3339 に適合しなければ schema 検証を通らない（§2）。Draft 2020-12 の既定（format = 注釈扱い）に依らず、niface の適合はこの評価を要求する（→ ADR-0025）
