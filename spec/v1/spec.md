...
- `specVersion` は整数。互換変更（フィールド追加）では増やさない。フィールドの削除・意味変更・必須化で増やす。ただし次の 2 類型は互換扱いとし `specVersion` を上げない。(a) 参照実装間で相互運用が成立していなかった領域（実装依存で id 等が割れていた入力域）を規範から除外・厳格化する変更（それに安全に依存できた消費側が存在しないため。→ §5 identity の値域）。(b) 全参照実装が既に一致した挙動を示している領域を、その挙動どおりに規範化（明文化）する変更。実装の挙動と id 導出結果は変わらず、消費側が観測する値も変わらないため、必須化（MUST の追加）を伴っても互換破壊にならない（→ §5 の `-0` 正規化・ADR-0027）
...
- `startedAt` / `finishedAt` は実行の開始・終了時刻を表す RFC 3339 の date-time 文字列でなければならない（MUST）。日付と時刻は `T`（または `t`）で区切り、タイムゾーンオフセット（`Z` または `±hh:mm`）を伴わなければならない（MUST）。`subjectResult` の `startedAt` / `finishedAt`（各主体の実行時刻）も同じ制約に従う。schema は `format: date-time` でこれを表現し、適合検証は format を assertion として評価する（§8）
...
identity（`kind` と `key`）を構成する JSON 値は次の値域に限る。この値域の上では JCS による正準化が全実装で一致し、id の言語間互換が保証される。域外の値を含む identity を、実装は id 導出時に**拒否しなければならない**（MUST）。

- 文字列: 値は全 Unicode を許す
- 数値: **整数表記**の整数のみを許し、値域は −(2^53−1) 以上 +(2^53−1) 以下とする。範囲外の整数、および小数点・指数を持つ表記（`1.0`・`1e3` 等、値が整数でも）・NaN・Infinity は域外とする（MUST NOT）。負ゼロ表記（`-0`）は域内で、`0` と同一の identity として扱う。実装は id 導出時に `0` へ正規化しなければならない（MUST）
- 真偽値（bool）／ null
- 配列: 要素は本値域の値
- オブジェクト: 値は本値域の値。**メンバー名は ASCII（U+0000–U+007F）に限る**

数値は**値ではなく表記**で判定する。`1` と `1.0` は別の入力として扱い、値が整数でも小数点・指数表記（`1.0`・`1e3`）は域外とする。参照実装はこれを機械的に一致させる（Nix は `builtins.fromJSON` の int / float 型、Go は `encoding/json` の `UseNumber`（`json.Number` の字句）で表記を保持する）。`-0` は整数表記のため域内とし、両参照実装が id 導出時に `0` へ正規化する（Nix の `builtins.fromJSON` は `-0` を整数 `0` として読み、Go は `json.Number("-0")` を整数 `0` として解釈する）。整数表記に符号付きゼロの区別は生まれないため、`-0` を `0` に畳んでも表記判定モデルと矛盾しない。整数の上限を安全整数（2^53−1、IEEE 754 倍精度が正確に表せる整数の上限）に揃えるのは、JSON の整数を double で読む実装との間で id が割れるのを防ぐためである。メンバー名を ASCII に限るのは、JCS のキー順序（UTF-16 code unit 順）と実装のキー順序（例: UTF-8 バイト順）が BMP 外の文字で食い違い、id が割れるのを避けるためである。文字列の値と配列・オブジェクトの値には非 ASCII を許す。key は宣言上の同一性を表す最小の値集合とする原則（本節）のもとで、この値域が実用上の制約になることはない。
